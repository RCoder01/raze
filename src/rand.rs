...
pub fn random_unit(rand: &mut impl RandSource) -> Vec3 {
    let dir = rand.rand::<f64>() * std::f64::consts::TAU;
    let height = rand.rand::<f64>() * 2. - 1.;
...
thread_local! {
    static THREAD_LCG: RefCell<JavaLcg> = RefCell::new(JavaLcg::from_time());
}
